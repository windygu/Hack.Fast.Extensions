...
namespace Hack.Fast.Extensions.CodeDom
{
...
namespace _Tool.AutoGenerateCode {";
		...
		private void Insert()
		{
			...
			foreach( DbMapInfo info in _allFields ) {
				if( info.AttrColumn != null && (info.AttrColumn.Identity || info.AttrColumn.TimeStamp || info.AttrColumn.SeqGuid) )
					continue;
		...
		private void UpdateSetFields()
		{
			...
			foreach( DbMapInfo info in _allFields ) {
				if( info.AttrColumn != null && (info.AttrColumn.Identity || info.AttrColumn.TimeStamp || info.AttrColumn.PrimaryKey) )
					continue;
