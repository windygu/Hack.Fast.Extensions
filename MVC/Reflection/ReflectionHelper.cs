...
namespace Mysoft.Map.MVC
{
	...
	internal static class ReflectionHelper
	{
		...
		private static void InitControllers()
		{
			...
			// 必须以【.Services.dll】结尾的程序集，才能被框架接受。
			string searchPattern = System.Configuration.ConfigurationManager.AppSettings["MapExt-MVC:ServiceDllSearchPattern"];
			if( string.IsNullOrEmpty(searchPattern) )
				searchPattern = ".Services.dll";

			foreach( Assembly assembly in assemblies ) {

				if(assembly.Location.EndsWith(searchPattern, StringComparison.OrdinalIgnoreCase)==false )
					continue;
		...
		private static MethodInfo FindAction(string action, Type controller, HttpRequest request)
		{
			...
			if( list.Count > 1 )
				throw new InvalidProgramException(
					string.Format("不允许在 {0} 定义多个同名的方法：{0} 。", controller.FullName, action));
		...
		/// <param name="ifNotFoundThrowException"></param>
		/// <returns></returns>
