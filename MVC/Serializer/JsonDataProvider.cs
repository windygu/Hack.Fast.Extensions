...
namespace Mysoft.Map.MVC.Serializer
{
	internal class JsonDataProvider : IActionParametersProvider
	{
		...
		private static readonly MethodInfo s_methodDeserialize
				= typeof(Hack.Fast.Extensions.Json.JsonHelper).GetMethod("JsonDeserialize",
		...
		public object[] GetParameters(HttpContext context, ActionDescription action)
		{
			...
			if( action.Parameters.Length == 1 ) {
				...
				throw new NotSupportedException("JSON序列化的结果只支持一个参数接收。");
		...
		public object GetObjectFromString(string input, ActionDescription action)
		{
			...
			return deserialize.FastInvoke(null, new object[] { input });
		}
